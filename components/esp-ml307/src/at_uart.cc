...
void AtUart::ResetReceiveState() {
    std::lock_guard<std::mutex> lock(rx_mutex_);
    uart_flush_input(uart_num_);
    rx_buffer_.clear();
    response_.clear();
    xEventGroupClearBits(event_group_handle_,
        AT_EVENT_DATA_AVAILABLE | AT_EVENT_COMMAND_DONE | AT_EVENT_COMMAND_ERROR |
        AT_EVENT_BUFFER_FULL | AT_EVENT_FIFO_OVF | AT_EVENT_BREAK);
}

bool AtUart::DetectBaudRate(int preferred_baud_rate) {
    int baud_rates[9] = {};
    size_t baud_rate_count = 0;
    auto add_baud_rate = [&](int rate) {
        for (size_t i = 0; i < baud_rate_count; ++i) {
            if (baud_rates[i] == rate) {
                return;
            }
        }
        baud_rates[baud_rate_count++] = rate;
    };
    add_baud_rate(115200);
    add_baud_rate(preferred_baud_rate);
    add_baud_rate(460800);
    add_baud_rate(921600);
    add_baud_rate(230400);
    add_baud_rate(57600);
    add_baud_rate(38400);
    add_baud_rate(19200);
    add_baud_rate(9600);
    static constexpr size_t detection_rounds = 3;

    vTaskDelay(pdMS_TO_TICKS(1500));
    for (size_t round = 0; round < detection_rounds; ++round) {
        ESP_LOGI(TAG, "Detecting modem baud rate...");
        for (size_t rate_index = 0; rate_index < baud_rate_count; ++rate_index) {
            const int rate = baud_rates[rate_index];
            if (uart_set_baudrate(uart_num_, rate) != ESP_OK) {
                continue;
            }
            vTaskDelay(pdMS_TO_TICKS(30));
            const size_t attempts_per_rate = rate == 115200 ? 5 : 2;
            const size_t at_timeout_ms = rate == 115200 ? 800 : 300;
            for (size_t attempt = 0; attempt < attempts_per_rate; ++attempt) {
                ResetReceiveState();
                if (SendCommand("AT", at_timeout_ms)) {
    ...
    ResetReceiveState();
    ESP_LOGE(TAG, "Failed to detect modem baud rate");
    return false;
}
