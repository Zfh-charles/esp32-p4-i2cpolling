...
def find_contract_root(script_path: Path) -> Path:
    """Support both the local workspace and the public tools/ layout."""
    parent = script_path.resolve().parent.parent
    for root in (parent / "product_contracts", parent / "tools" / "product_contracts"):
        if (root / "validate_contracts.py").is_file():
            return root
    raise FileNotFoundError("Missing product_contracts/validate_contracts.py; keep the PC tool and contracts together")
