...
from character_pack_compiler import CANONICAL_EMOTIONS, compile_character_pack, find_contract_root, validate_profile, validate_v3


class CharacterPackCompilerTest(unittest.TestCase):
    def test_contract_lookup_supports_workspace_and_public_layouts(self) -> None:
        with tempfile.TemporaryDirectory() as value:
            root = Path(value)
            contracts = root / "tools" / "product_contracts"
            contracts.mkdir(parents=True)
            (contracts / "validate_contracts.py").write_text("# fixture", encoding="utf-8")
            for tool in (root / "emotion_tool_dev", root / "tools" / "emotion_tool_dev"):
                self.assertEqual(find_contract_root(tool / "character_pack_compiler.py"), contracts)

    def test_contract_lookup_rejects_missing_validator(self) -> None:
        with tempfile.TemporaryDirectory() as value:
            root = Path(value)
            (root / "tools" / "product_contracts").mkdir(parents=True)
            with self.assertRaisesRegex(FileNotFoundError, "Missing product_contracts"):
                find_contract_root(root / "emotion_tool_dev" / "character_pack_compiler.py")
