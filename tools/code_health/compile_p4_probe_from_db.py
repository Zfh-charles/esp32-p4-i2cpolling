...
def main() -> int:
    ...
    if output.exists() or output.with_suffix(output.suffix + ".rsp").exists():
        raise SystemExit("probe output/response already exists; use a new probe directory")
    ...
    if args.dry_run:
        print(f"P4_PROBE_PLAN NON_FLASHABLE source={source} output={output} commands=1")
        return 0
    ...
    print(f"P4_PROBE_COMPILE PASS V0_TU_ONLY NON_FLASHABLE source={source.name} output={output}")
    return 0
